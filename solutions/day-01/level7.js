...
for (const key in users) {
  ...
  if (curUserSkills.includes("MongoDB") && curUserSkills.includes("Express") && curUserSkills.includes('React') && curUserSkills.includes('Node')) {
    mernDevs.push(key)
  }
}
...
users.Leon = {
  email: 'leon@leon.com',
  skills: ['Python', 'Express', 'Node', 'Java', 'HTML', 'CSS', 'JavaScript', 'React', 'Angular'],
  age: 23,
  isLoggedIn: false,
  points: 50
}
...
for (const key in countries) {
  ...
  console.log(`${name}'s capital is ${capital}. The population count is ${population} and they speak ${language}`)
}